...
from jax_rnafold.common.utils import (
    MAX_LOOP,
    MAX_PRECOMPUTE,
    NBPS,
    N4,
...
def _ensure_jax_energy_model(em: energy.Model) -> energy.Model:
    if getattr(em, "jaxify", True):
        return em

    if isinstance(em, energy.StandardNNModel):
        return energy.JaxNNModel(
            params_path=em.nn_params.params_path,
            max_precompute=getattr(em, "max_precompute", MAX_PRECOMPUTE),
            temp=em.temp,
            hairpin=em.hairpin,
        )

    raise TypeError(
        "compute_inside_tables requires a JAX-compatible energy model. "
        "Instantiate `energy.JaxNNModel` or set `jaxify=True` on the provided model."
    )


def _build_inside_tables_fn(
    em: energy.Model,
    seq_len: int,
    max_loop: int,
    scale: float,
    checkpoint_every: Optional[int],
) -> Tuple[Callable[[jnp.ndarray], Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]], jnp.ndarray, int]:
    ...
    s_table = jnp.array([math.exp(i * scale / seq_len) for i in range(seq_len + 5)], dtype=f64)

    special_hairpin_lens = em.special_hairpin_lens
    max_sp_hairpin_len_up = special_hairpin_lens.max() - 2
    ...
    def fill_external(i, E, P, padded_p_seq):
        def get_j_bp_term(j, bp_idx):
            ...
            return jnp.where(
                cond,
                base_en * P[bp_idx, i, j] * em.en_ext_branch(bi, bj) * s_table[2],
                0.0,
            )
    ...
    def fill_outer_mismatch(i, OMM, P, padded_p_seq):
        ...
        compute_all_terms = vmap(
            vmap(vmap(compute_term, (None, 0, None, None)), (None, None, 0, None)),
            (None, None, None, 0),
        )
    ...
    def psum_hairpin_special_correction(bi, bj, i, j, padded_p_seq):
        ...
        def special_hairpin_correction(id):
            ...
            correction = (
                pr_special_hairpin(id, i, j, padded_p_seq)
    ...
    def psum_hairpin_not_special(bi, bj, i, j, padded_p_seq):
        ...
        def u1_fn(bip1):
            return padded_p_seq[i + 1, bip1] * em.en_hairpin_not_special(bi, bj, bip1, bip1, 1)
        ...
        return jnp.where(
            u == 0,
            em.en_hairpin_not_special(bi, bj, bj, bi, 0),
            jnp.where(u == 1, jnp.sum(u1_fn(N4)), jnp.sum(u_general_fn(N4, N4))),
        )

    @jit
    def psum_hairpin(bi, bj, i, j, padded_p_seq):
        return (
            psum_hairpin_not_special(bi, bj, i, j, padded_p_seq)
    ...
    def psum_internal_loops(bi, bj, i, j, padded_p_seq, P, OMM):
        ...
        def get_bp_1n_sm(bp_idx, bip1, bjm1):
            ...
            def z_b_fn(z_offset, b):
                ...
                l = j - 3 - z_offset
                l_cond = l >= i + 3
                il_en = em.en_internal(bi, bj, bk, bl, bip1, bjm1, bip1, b, 1, j - l - 1)
                right_term = (
                    P[bp_idx, i + 2, l]
                    * padded_p_seq[i + 2, bk]
                    * padded_p_seq[l, bl]
                ...
                k = i + 3 + z_offset
                k_cond = k < j - 2
                il_en = em.en_internal(bi, bj, bk, bl, bip1, bjm1, b, bjm1, k - i - 1, 1)
                left_term = (
                    P[bp_idx, k, j - 2]
                    * padded_p_seq[k, bk]
                    * padded_p_seq[j - 2, bl]
        ...
        get_all_1n_terms = vmap(
            vmap(vmap(get_bp_1n_sm, (0, None, None)), (None, 0, None)), (None, None, 0)
        )
        ...
        get_all_special_terms = vmap(
            vmap(vmap(get_bp_22_23_32_sm, (0, None, None)), (None, 0, None)), (None, None, 0)
        )
        sm += jnp.sum(
            get_all_special_terms(jnp.arange(NBPS), jnp.arange(3), jnp.arange(3))
        )
    ...
    def compute_tables(p_seq: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]:
        ...
        E = E.at[seq_len].set(1.0)
        ML = ML.at[0, :, :].set(1.0)

        @jit
        def fill_table(carry, i):
            OMM_c, P_c, ML_c, MB_c, E_c = carry

            P_c = fill_paired(i, padded_p_seq, OMM_c, ML_c, P_c)
            OMM_c = fill_outer_mismatch(i, OMM_c, P_c, padded_p_seq)
            MB_c = fill_multibranch(i, MB_c, P_c, padded_p_seq)
            ML_c = fill_multi(i, padded_p_seq, ML_c, MB_c)
            E_c = fill_external(i, E_c, P_c, padded_p_seq)

            return (OMM_c, P_c, ML_c, MB_c, E_c), None

        (OMM, P, ML, MB, E), _ = scan(
            fill_table, (OMM, P, ML, MB, E), jnp.arange(seq_len - 1, -1, -1)
        )

        return padded_p_seq, OMM, P, ML, MB, E

    return compute_tables, s_table, two_loop_length
...
def compute_inside_tables(
    ...
    em = _ensure_jax_energy_model(em)

    p_seq = jnp.array(seq_to_one_hot(seq), dtype=f64)
    compute_tables, s_table, two_loop_length = _build_inside_tables_fn(
        em, seq_len, max_loop, scale, checkpoint_every
    )

    padded_p_seq, OMM, P, ML, MB, E = compute_tables(p_seq)
    padded_p_seq, OMM, P, ML, MB, E, s_table = jax.device_get(
        (padded_p_seq, OMM, P, ML, MB, E, s_table)
    )

    return InsideTables(
        seq_len=seq_len,
        two_loop_length=two_loop_length,
        padded_p_seq=np.asarray(padded_p_seq),
        E=np.asarray(E),
        P=np.asarray(P),
        ML=np.asarray(ML),
        MB=np.asarray(MB),
        OMM=np.asarray(OMM),
        s_table=np.asarray(s_table),
        scale=scale,
    )
